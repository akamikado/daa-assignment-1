...
public:
  Graph(int vertices) : n(vertices) { adj.resize(n); }
  ...
  const unordered_set<int> &getNeighbors(int v) const { return adj[v]; }
...
private:
  ...
  void BronKerboschPivot(vector<int> &R, vector<int> &P, vector<int> &X) {
    ...
    for (int v : P) {
      if (pivot == -1 ||
          G.getNeighbors(pivot).find(v) == G.getNeighbors(pivot).end()) {
...
public:
  ...
  void printResults() const {
    ...
    if (!maximalCliques.empty()) {
      largestCliqueSize = max_element(maximalCliques.begin(),
                                      maximalCliques.end(), compareBySize)
                              ->size();
    }
...
int main(int argc, char *argv[]) {
  ...
  bk.printResults();
  cout << "\nExecution time: " << fixed << setprecision(3) << elapsed.count()
       << " seconds" << endl;

  string csv_filename = "els" + filename + ".csv";
